// app/dashboard/projects/[id]/folders/[folderId]/page.tsx
// @ts-nocheck
...
export async function generateMetadata({
  ...
  return {
    ...
    description: "Folder workspace for media review and collaboration",
  };
}

// Main page component
export default async function FolderWorkspacePage({
  params,
}: {
  params: Promise<{ id: string; folderId: string }>;
}) {
  ...
  if (!user) {
    const returnUrl = encodeURIComponent(
      `/dashboard/projects/${projectId}/folders/${folderId}`
    );
          ...
          <CardContent className="pt-8 pb-8 px-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Button variant="outline" asChild className="flex-1">
                <Link
                  href={`/dashboard/projects/${projectId}/folders/${folderId}`}
                >
        ...
        <Card className="w-full max-w-md">
          <CardContent className="pt-8 pb-8 px-8 text-center">
            ...
            <div className="space-y-4 mb-8">
              ...
              <p className="text-muted-foreground leading-relaxed">
                The requested folder does not exist or you don't have access to
                it.
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
  ...
  // Get ALL media files for the project to populate subfolder thumbnails
  ...
  // Get folder media data for current folder
  const folderMediaResult = await getFolderMediaAction(projectId, folderId);
  const mediaFiles = folderMediaResult.success
    ? folderMediaResult.mediaFiles || []
    : [];
  const reviewLinks = folderMediaResult.success
    ? folderMediaResult.reviewLinks || []
    : [];

  // Process folders with media information for subfolder thumbnails
  ...
  const mediaFilesWithDisplayOrder = mediaFiles.map((media) => ({
    ...media,
  ...
  // Create enhanced project object with folder-specific data
  const enhancedProject = {
    ...project,
    project_media: mediaFilesWithDisplayOrder,
    review_links: reviewLinks,
    project_members: membersWithProfiles,
    project_folders: processedFolders, // Use processed folders with media info
  ...
  return (
    <FolderWorkspace
      ...
      allFolders={processedFolders} // Pass processed folders with media
