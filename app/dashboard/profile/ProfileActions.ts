...
// Constants updated for profile naming
const PROFILE_AVATAR_MAX_SIZE = 32 * 1024 * 1024; // 32MB
...
export async function updateProfile(formData: FormData) {
  ...
  try {
    ...
    if (avatarFile instanceof File) {
      if (avatarFile.size > PROFILE_AVATAR_MAX_SIZE) {
    ...
    revalidatePath("/dashboard"); // Also revalidate dashboard for sidebar updates
