...
// Account actions.ts files
import { createClient } from "@/utils/supabase/server";
import { revalidatePath } from "next/cache";

// Constants

const AVATAR_MAX_SIZE = 5 * 1024 * 1024; // 5MB for avatar
  ...
  try {
    ...
    if (avatarFile instanceof File) {
      if (avatarFile.size > AVATAR_MAX_SIZE) {
        throw new Error(
          `Avatar file size exceeds ${AVATAR_MAX_SIZE / (1024 * 1024)}MB limit`
        );
      }
    ...
    for (const [key, value] of formData.entries()) {
      ...
      if (jsonFields.includes(key)) {
        try {
          ...
          console.warn(`Failed to parse ${key} as JSON:`, error);
    ...
    // Check display name availability if changed
    if (
